...
**v4.5+ prefers natural language descriptions over keyword lists.**
Put genre + vocal + mood + key instruments in the FIRST 20-30 words.
Keep total under 1,000 chars (silent truncation beyond that).
Stick to 1-2 genres / 2-3 instruments / 1-2 moods — more = generic.
...
KOREAN VOCALS (v5+): Add `clear pronunciation` or `high fidelity vocals`
to style prompt. Always write lyrics in Hangul, never romanized.
...
- Use Exclude Styles field for what you DON'T want (max 200 chars)
  Example: `autotune, choir, EDM drums, crowd vocals, synth`
...
- Use Extend to build structure: generate a good verse+chorus first,
  then Extend to add bridge/outro (preserves mix/tone at ~99%)
- Generate 3-5 variants per song — treat them like recording takes
...
### v4.5+ / v5 / v5.5 Features (2025)

Suno has evolved significantly. Key capabilities:
- **v4.5**: 1,000-char style field (was 200), 8-min songs, natural language prompts work better than keyword lists
- **v5**: Korean/multilingual pronunciation overhaul — always use v5+ for Korean vocals
- **v5.5**: Voice Persona — upload 3-5 min dry vocal to clone your voice
- **Extend**: Append sections to existing songs (preserves tone at ~99%)
- **Cover**: Same melody/structure, different genre/vocals/instruments
- **Vocal Swap**: Swap vocalist while keeping lyrics + melody
- **Instrumental Flip**: Change backing genre while keeping vocals
- **Spark from Playlist**: Feed your best songs as reference for new generation
- **Stem Extraction Pro**: Up to 12 stems (drums, bass, vox, guitar, piano, etc.)
- **Exclude Styles**: Negative prompt field — specify what you DON'T want (max 200 chars)

See `references/suno-v45-v5-features.md` for full feature details and production pipeline.
See `references/post-processing-pipeline.md` for free DAW options, Demucs API patterns (pitfalls!), Pedalboard usage, and the 8-genre postprocess.py pipeline.
Working pipeline: `~/music-lab/scripts/postprocess.py` — `python postprocess.py input.wav --genre jazz`
Full advanced guide: `~/music-lab/docs/suno-advanced-guide.md`
...
- v4.5+ supports timing in tags: `[Solo: Tenor Sax, 16s]`, `[Intro - piano, 12s]`
- Backup vocal syntax: `{backup vocals: "ohh ahh"}`
- Inline delivery: `(whispered)`, `(belted)`, `(falsetto)`
