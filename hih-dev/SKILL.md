...
STEP 1.5  병렬 분해 분석   — 독립 서브태스크 분해 가능 여부 판단 (신규)
STEP 2    /office-hours   — 아이디어/설계 검토 (이미 충분히 논의했으면 SKIP)
...
## STEP 2 — /office-hours (조건부)

**SKIP 조건** (하나라도 해당하면 건너뜀):
- 이미 이 기능을 충분히 논의한 적 있다
- CLAUDE.md 또는 태스크 비고에 설계가 명확히 적혀있다
- 단순 버그픽스 / 명확한 구현 태스크다

**실행 조건**:
- 새 기능인데 방향이 불명확하다
- "이걸 왜 만드는지" 다시 점검이 필요하다
- 사용자가 명시적으로 요청했다
